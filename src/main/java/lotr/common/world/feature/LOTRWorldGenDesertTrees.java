...
extends WorldGenAbstractTree {
    ...
    public boolean generate(World world, Random random, int i, int j, int k) {
        ...
        isSoil = (below = world.getBlock(i, j - 1, k)).canSustainPlant((IBlockAccess)world, i, j - 1, k, ForgeDirection.UP, (IPlantable)Blocks.sapling) || this.isNatural && (below == Blocks.sand || below == Blocks.stone);
        if (!isSoil) {
            flag = false;
        }
        if (!flag) {
