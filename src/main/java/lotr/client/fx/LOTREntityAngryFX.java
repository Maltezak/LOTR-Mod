...
public class LOTREntityAngryFX
extends EntityFX {
    ...
    public void renderParticle(Tessellator tessellator, float f, float f1, float f2, float f3, float f4, float f5) {
        float modScale = LOTRFunctions.normalisedCos((this.particleAge + f) / 12.0f);
        modScale = MathHelper.clamp_float(modScale, 0.0f, 1.0f);
        this.particleScale = this.angryScale * (0.7f + modScale * 0.3f);
        float uMin = this.particleTextureIndexX / 4.0f;
        float uMax = uMin + 0.25f;
        float vMin = this.particleTextureIndexY / 4.0f;
