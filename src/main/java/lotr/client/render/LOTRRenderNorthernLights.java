...
public class LOTRRenderNorthernLights {
    ...
    private static boolean atUtumno;
    private static int utumnoChange;
    private static final Random rand;
    private static final Random dateRand;
    ...
    private static float getNorthernness(EntityLivingBase entity) {
        ...
        northernness = MathHelper.clamp_float(northernness, 0.0f, 1.0f);
        return northernness;
    }
    ...
    public static void update(EntityLivingBase viewer) {
        ...
        float daytime = world.getWorldTime() % LOTRTime.DAY_LENGTH;
        if (daytime < 0.25f) {
            --effectiveDay;
        }
    ...
    private static class AuroraCycle {
        ...
        public float calc(float t, float tick) {
            return MathHelper.cos(t * this.freq + tick * this.tickMultiplier) * this.amp * this.ampModifier;
        }
